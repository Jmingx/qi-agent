...
**检查器不预判"逻辑问题"，只预判"类型安全问题"。**

| 检查器能管（类型安全） | 检查器不能管（业务逻辑） |
|----------------------|------------------------|
| 对可能为 None 的值取下标 | 算法对不对、结果是否想要 |
| 把字符串当数字用 | 业务规则（金额不能为负等） |
| 调用了不存在的方法/属性 | 边界条件、空列表、异常流程 |
| 传错了参数类型 | 性能、并发问题 |

**"盯着你先看一眼"的具体机制（类型收窄 type narrowing）：**

```python
def f(result: ChatResult):
    first = result.tool_calls[0]    # ① 警告：可能是 None，直接下标会崩
    if result.tool_calls:           # ② 判空后，分支内类型被"收窄"为纯 list
        return result.tool_calls[0].name   # ✅ 安全，不警告
```

**类比：检查器像"交规"不是"导航"**——抓"超速"（类型隐患）这类危险行为，不管"你要去哪"（业务逻辑）。

**双保险体系：**
- 检查器（mypy/pyright）：静态，抓类型隐患（编译前）
- 测试（pytest）：动态，抓逻辑错误（运行中）
- 两者互补，各管一段。**逻辑正确性永远要靠测试和自己思考。**
