...
| ⬜ | **长对话评测（L1-L4 保障）** | P2 | ⭐⭐ | 评测集加长对话用例：连跑 50 轮后回答质量不下降、token 不超限、压缩后关键事实仍记得（与 evaluation.md"长对话与上下文管理评测 L1-L4"条目联动——L3 事实保持/ L4 质量回归走这里，L1/L2 协议/有界性进 CI） |
...
- **算法选型（2026-08-22 讨论，方案 docs/plans/2026-08-22-上下文管理方案.md 已写待评审，实施挂起）**：
  - **裁剪第一版 4 算法**：①token 预算（数 token 不数条数）②消息组裁剪（tool_calls+结果成对）③role 交替边界 ④优先级保留（system/sticky 永不裁）+ anchor 占位提示
  - **压缩第一版 2 算法**：①结构化摘要（关键事实/用户要求/决策/未完成任务/工具结果结论——对齐 Hermes）②最近窗口保留（摘要 + 最近 N 条原文）
  - **演进项（按需再加）**：冗余合并 / 分段压缩（超长分块）/ 递归压缩（摘要再摘要）/ 聚焦压缩（/compact <focus>，对齐 Hermes）/ 问答对提炼 / 事实清单抽取 / 预算驱动触发 / preflight 预检
  - **业界对照**：Hermes 结构化摘要+聚焦压缩最全；Claude Code 滑动窗口+摘要；OpenAI 纯裁剪；DSH compaction 基础压缩
  - **测评保障已定（L1-L4）**：L1 协议正确性 / L2 有界性（mock 进 CI）/ L3 事实保持 / L4 质量回归（真实 LLM 走评测集）——见 evaluation.md"长对话与上下文管理评测 L1-L4"条目
