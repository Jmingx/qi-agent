...
| ⬜ | **审批交互 stdin 竞争（2026-08-28 真实 bug）** | P1 | ⭐⭐⭐ | 审批弹窗（clarify 在 worker/工具线程 input）与主线程 input 竞争 stdin——审批期间用户输入可能被主循环抢走 → 同 context 并发 run → 消息污染 400。**已做**：manager.run 并发防护（RUNNING 拒绝）。**未做**：审批交互移到主线程/异步交互层（InteractionProvider 改造——ask 不阻塞 worker，主线程统一处理）。触发：审批弹窗 + 用户快速输入 |
...
| ⬜ | **Gateway 补异步能力：session/status 查询（Web 铺路，2026-08-30）** | P0 | ⭐⭐ | Gateway 新增 RPC 方法 `session/status`（查询 context 状态 + 结果）：返回 status/phase/turn/result/error。Web 界面轮询用（拉起子任务后定期查——不用阻塞等）。实现：manager.get_context + context.poll/result。**背景**：/delegate 当前同步等（CLI 体验），但 Web 需要异步轮询——内核提供原子能力（spawn 异步 + status 查询），shell 自己编排等待策略 |
| ⬜ | **_delegate 拆纯异步（Web 铺路，2026-08-30）** | P0 | ⭐⭐ | `_delegate` 去掉内部同步 `sub.wait`（当前写死同步——CLI 阻塞主进程，用户无法插话/steer）。改为：spawn 拉起即返回 `{"session_id": 子id, "status": "spawned"}`，结果由 `session/status` 轮询获取。CLI 侧用同步包装（轮询 status 等完成——保持现有体验）；Web 侧天然异步（拉起 + 轮询）。**触发**：/delegate 期间用户输入补充指令无消息（steer 无调用方 + CLI 阻塞）——换 Web 后解决。**注意**：修复 `sub.wait(timeout=0)` bug（当前 wait(0) 立即超时返回 None——同步等待彻底失效） |
