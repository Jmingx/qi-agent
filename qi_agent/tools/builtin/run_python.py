...
def _approval_condition(arguments: dict) -> "str | ToolDecision | None":
    """run_python 审批条件（v0.4.26 声明式；决策码机制 2026-08-23）。

    import 受限白名单外模块 → SANDBOX 档（结构化决策，approval_gate 按
    code=SEC_APPROVAL_SANDBOX 弹降级透明提示，无 a=总是允许）；
    否则 None（放行）。工具自声明权限策略——security_guard 查 registry 执行。
    """
    ...
    for m in re.finditer(r"^\s*(?:import|from)\s+([\w.]+)", code, re.M):
        ...
        if module not in _RESTRICTED_MODULES:
            return ToolDecision(
                ...
                reason=f"代码需要 import '{module}'（受限环境白名单外），需降级审批",
                code=SEC_APPROVAL_SANDBOX,
                command=f"import '{module}'（沙箱降级）",
            )
    return None
