...
def _approval_condition(arguments: dict) -> "str | ToolDecision | None":
    """shell 审批条件（v0.4.26 声明式；决策码机制 2026-08-23）。

    命令前缀匹配（对齐 security_guard 判档语义）：
    - 代码执行类（python/node/npm/pip 等）→ ESCALATION 档（独立 action）：
      以完整权限运行（不受 run_python 沙箱约束），approval_gate 按
      code=SEC_APPROVAL_ESCALATION 弹"完整权限"透明提示（无 a=总是允许）
      ——不再用"沙箱升级:"字符串前缀（2026-08-23 决策码机制）
    - 需审批命令（rm/git push 等）→ 普通审批档（str → security_guard
      包成 SEC_APPROVAL_GENERAL）
    ...
    if any(lowered.startswith(p) for p in CODE_EXEC_PREFIXES):
        # ESCALATION 档（2026-08-23 新档）：结构化决策替代"沙箱升级:"前缀
        return ToolDecision(
            ToolAction.ESCALATION,
            reason=f"命令以完整权限执行（不受沙箱约束）: {command}",
            code=SEC_APPROVAL_ESCALATION,
            command=command,
        )
